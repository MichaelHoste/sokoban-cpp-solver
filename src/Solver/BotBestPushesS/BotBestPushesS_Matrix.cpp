...
#include "../../../include/Solver/BotBestPushesS/BotBestPushesS_Matrix.h"
...
BotBestPushesS_Matrix<T>::BotBestPushesS_Matrix() {
	m_rows = 0;
	m_columns = 0;
	m_matrix = NULL;
}
...
/*export*/ template <class T>
BotBestPushesS_Matrix<T>::BotBestPushesS_Matrix(const BotBestPushesS_Matrix<T> &other) {
...
/*export*/ template <class T>
BotBestPushesS_Matrix<T>::BotBestPushesS_Matrix(int rows, int columns) {
	m_matrix = NULL;
	resize(rows, columns);
}

/*export*/ template <class T>
BotBestPushesS_Matrix<T> &
...
/*export*/ template <class T>
void
BotBestPushesS_Matrix<T>::resize(int rows, int columns) {
	if ( m_matrix == NULL ) {
		// alloc arrays
...
/*export*/ template <class T>
void
BotBestPushesS_Matrix<T>::identity() {
...
/*export*/ template <class T>
BotBestPushesS_Matrix<T>&
BotBestPushesS_Matrix<T>::transpose() {
	assert( m_rows > 0 );
	assert( m_columns > 0 );
...
BotBestPushesS_Matrix<T>::product(BotBestPushesS_Matrix<T> &other) {
	...
	BotBestPushesS_Matrix<T> out(m_rows, other.m_columns);
...
/*export*/ template <class T>
T&
BotBestPushesS_Matrix<T>::operator ()(int x, int y) {
	assert ( x >= 0 );
	assert ( y >= 0 );
	assert ( x < m_rows );
	assert ( y < m_columns );
